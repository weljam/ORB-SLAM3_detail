...
public:
    ...
    //Nga is noise Matrix , it describe the noise of sensor 
    // while NgaWalk describes the noise of the constant velocity motion model,
    // so calls process noise Matrix
    Eigen::DiagonalMatrix<float,6> Nga, NgaWalk;

    // Values for the original bias (when integration was computed)
    Bias b;
    // representing the rotation increment of the pre-integration
    Eigen::Matrix3f dR;
    // representing the velocity increment ,and the position increment
    Eigen::Vector3f dV, dP;
    // Jacob Matrix dR/dg , dV/dg, dV/da ,dP/dg ,Dp/da,
    //which a，g is all bias, b^a,b^g
    Eigen::Matrix3f JRg, JVg, JVa, JPg, JPa;
    //average acceleration, average angle velocity.
