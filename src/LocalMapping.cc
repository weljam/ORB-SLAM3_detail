...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA)
{
    ...
    {
        Eigen::Matrix3f Rwg;//重力方向到世界坐标系的旋转矩阵
        Eigen::Vector3f dirG;//代求的重力方向
        dirG.setZero();
        ...
        {   
            ...
            // 其中GetUpdatedDeltaVelocity就是速度的预计分，当乘上Ri时，则Ri*Ri.T()=I
            // 所以有下述的式子为:dirG -= s*Vj-s*Vi-Rwg*g*tij
            // 由于是从初始帧到当前帧，则其中会消去过程中的Vj和Vi,dirG = sV1 - sVn + n*Rwg*g*t
