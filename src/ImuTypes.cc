...
/** 
 * @brief 计算右雅可比的逆x<double, 24, 24> Get
 * @param xyz so3
 * @return Jr^-1
 */
...
IntegratedRotation::IntegratedRotation(const Eigen::Vector3f &angVel, const Bias &imuBias, const float &time)
{
    ...
    {   
        //这里是罗德里格斯公式的变形，W*W与nn^T之间相差一个I对角矩阵即I+W*W/theta^2 = n*n^T  （1）
        //而罗德里格公式： R=cos(theta)I+(1-cos(theta))nn^T+sin(theta)n^  （2）  这里的theta即为d
        //将（1）代入（2）中可以得到对应的R = I + W*sin(theta)/theta + W*W(1-cos(theta))/theta^2 
        deltaR = Eigen::Matrix3f::Identity() + W * sin(d) / d + W * W * (1.0f - cos(d)) / d2;
        //李代数近似形式，Jl = sin(theta)/theta I +(1-sin(theta)/theta)aa^T+(1-cos(theta))/theta a^
        // Jr(THETA) = Jl(-THETA) 看slam14讲 P82 （4.31）进行推导
        // 推导结果在orb-slam源码剖析中的P348 （15-8）
...
void Preintegrated::IntegrateNewMeasurement(const Eigen::Vector3f &acceleration, const Eigen::Vector3f &angVel, const float &dt)
{
    ...
    //这里可以查找ORB-slamp359对应公式。Wacc = (a-bias)^
